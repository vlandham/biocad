...
import flare.vis.data.DataSprite;
	...
	/**
	 * Small modification of the DargControl to ensure that the control works together with the 
	 * PanZoomControl. For that the MOUSEDOWN event propagation has to be
	 * stopped only if the event occured on a target which fulfills the
	 * filter criteria
	 * 
	 ...
	 * them.
	 */
	class DragControlMod extends Control
	{
		 ...
		 */		
		public function DragControlMod(filter:*=null) {
			this.filter = filter;
		}
		...
		private function onMouseDown(event:MouseEvent) : void {
			...
			if (_filter==null || _filter(s)) {
				...
				event.stopPropagation(); //modification: applying this only if the target fulfills the filter
			}
